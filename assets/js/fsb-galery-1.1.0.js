...
(function ($) {
	...
	var FSBGalery = {
		...
		addThumbnail: function (indice, item) {
			...
			switch (item.element) {
				case 'image':
					thumbnailContent = $('<img/>', {
						src: ((this.settings.thumbnailsDirectory.length > 0) ? this.settings.thumbnailsDirectory : item.folder) + item.filename + item.extension,
						alt: item.description
					});
					break;
				case 'video':
					thumbnailContent = $('<img/>', {
						src: this.settings.thumbnailVideo,
						alt: item.description
					});
					break;
				case 'audio':
					thumbnailContent = $('<img/>', {
						src: this.settings.thumbnailAudio,
						alt: item.description
					});
					break;
				case 'block':
					thumbnailContent = $('<img/>', {
						src: this.settings.thumbnailBlock,
						alt: item.description
					});
					break;
				default:
			...
			thumbnailContent.appendTo(thumbnail);
	...
	$.fn.FSBGalery = function (config) {
		var defaults = {
			...
			thumbnailAudio: 'assets/images/audio-thumbnail.png',
			thumbnailBlock: 'assets/images/block-thumbnail.png',
			// Diaporama config
			autoplay: false,
